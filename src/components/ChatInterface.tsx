...
const ChatInterface = ({ chatId, initialMessage }: ChatInterfaceProps) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Track compelete response from the server for saving to database
    let fullResponse = ""
    ...
    try {
      ...
      // Extract the response data and handle the stream response
      ...
      // Process the stream reponse using the helper process function
      await processStream(reader, async (chunk) => {
        ...
        for (const msg of messages) {
          // initalize switch case for each message type
          switch (msg.type) {
            case StreamMessageType.Token: {
              // Add the token to the streamResponse
              if ("token" in msg) {
            ...
            case StreamMessageType.ToolStart: {
              // handle start of tool execution (e.g API call, file opearation)
              if ("tool" in msg) {
                setCurrentTool({
                  name: msg.tool,
                  input: msg.input,
            ...
            case StreamMessageType.ToolEnd: {
              // handle the completion of tool execution
              if ("tool" in msg && currentTool) {
                // Replace the "Processing..." message with the actual output
                ...
                // Update the currentTool to null
                setCurrentTool(null)
              }
              break
            }
            ...
            case StreamMessageType.Done: {
              // handle the completion of tool execution or entire message
              const assitantMessage: Doc<"messages"> = {
                _id: `temp_assitant_${Date.now()}`,
                chatId,
    ...
    <main className="flex-1 flex flex-col h-[calc(100vh-theme(spacing.16))]">
      ...
      <section className=" flex-1 overflow-y-auto bg-gray-50 p-2 md:p-0">
        <div>
          ...
          {/* Streamed Response: this is the response from the model as it is being generated */}
          {streamResponse && (
            <MessageBubble content={streamResponse} isUser={false} />
          )}
